...
class TemplateTagsTest(TestCase):
    ...
    @mock.patch(
        "django_pwa_assets.templatetags.pwa_assets.get_or_generate_icons",
        side_effect=mock_get_or_generate_icons,
    )
    def test_pwa_icons_tag(self, mock_icons):
        html = self.render_template("{% load pwa_assets %}\n{% pwa_icons 'logo.png' %}")
        self.assertIn('<link rel="icon"', html)
        self.assertIn('href="http://example.com/media/pwa/any-192x192.png"', html)
    ...
    @mock.patch(
        "django_pwa_assets.templatetags.pwa_assets.get_or_generate_splashes",
        side_effect=mock_get_or_generate_splashes,
    )
    def test_pwa_splashes_tag(self, mock_splashes):
        ...
        self.assertIn('href="http://example.com/media/pwa/splash-1125x2436.png"', html)
        self.assertIn('media="(device-width: 375px)"', html)

    @mock.patch(
        "django_pwa_assets.templatetags.pwa_assets.get_or_generate_favicons",
        side_effect=mock_get_or_generate_favicons,
    )
    def test_pwa_favicons_tag(self, mock_favicons):
        ...
        self.assertIn('href="http://example.com/media/pwa/favicon.ico"', html)

    @mock.patch(
        "django_pwa_assets.templatetags.pwa_assets.get_or_generate_mstiles",
        side_effect=mock_get_or_generate_mstiles,
    )
    def test_pwa_mstiles_tag(self, mock_mstiles):
        ...
        self.assertIn('content="http://example.com/media/pwa/mstile-150x150.png"', html)
        self.assertIn('<meta name="msapplication-TileColor" content="#ffffff"', html)

    @mock.patch(
        "django_pwa_assets.templatetags.pwa_assets.get_or_generate_all",
        side_effect=mock_get_or_generate_all,
    )
